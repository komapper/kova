...
class KovaPropertiesTest : FunSpec({
    ...
    context("kova.map") {
        ...
        test("onEachKey") {
            val result = Kova.map<String, Int>().onEachKey(Kova.string().min(2))
        ...
        test("onEachValue") {
            val result = Kova.map<String, Int>().onEachValue(Kova.int().positive())
    ...
    context("kova.or") {
        test("or") {
            val result = Kova.int().positive().or { Kova.int().lt(0) }.tryValidate(0)
            result.isFailure().mustBeTrue()
            val message = result.messages.single()
