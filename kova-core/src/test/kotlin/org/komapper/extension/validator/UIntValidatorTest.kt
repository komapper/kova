    ...
    FunSpec({
        ...
        context("plus") {
            ...
            test("failure") {
                ...
                result.messages[0].text shouldBe "Number 15 must be less than or equal to 10"
            }
        }
        ...
        context("max") {
            ...
            test("failure with value greater than threshold") {
                ...
                result.isFailure().mustBeTrue()
                result.messages[0].text shouldBe "Number 11 must be less than or equal to 10"
            }
        }
        ...
        context("chaining multiple validators") {
            ...
            test("failure with value 5") {
                val result = validator.tryValidate(5u)
                result.isFailure().mustBeTrue()
                result.messages[0].text shouldBe "Number 5 must be greater than 6"
            }

            test("failure with value 10") {
                val result = validator.tryValidate(10u)
                result.isFailure().mustBeTrue()
                result.messages[0].text shouldBe "Number 10 must be less than or equal to 9"
            }
        }
    })
