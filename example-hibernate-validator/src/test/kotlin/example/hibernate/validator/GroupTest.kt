...
class GroupTest :
    FunSpec({
        ...
        context("kova") {
            ...
            fun Person.validate(checks: Set<Check> = setOf(Check.DEFAULT)) =
                schema {
            ...
            context(_: Validation)
            fun Driver.validate(checks: Set<Check> = setOf(Check.DEFAULT)) =
                schema {
                    (this@validate as Person).validate(checks)
            ...
            context(_: Validation)
            fun Car.validate(checks: Set<Check> = setOf(Check.DEFAULT)) =
                schema {
                    if (Check.DEFAULT in checks) {
                        ::manufacturer {
                            it.ensureNotNull()
            ...
            test("driveAway") {
                ...
                var result = tryValidate { car.validate() }
                result.shouldBeSuccess()

                // but ensureHas it passed the vehicle inspection?
                result = tryValidate { car.validate(setOf(Check.CAR)) }
                ...
                john.age = 18
                car.driver = john
                result = tryValidate { car.validate(setOf(Check.DRIVER)) }
                result.shouldBeFailure()
                result.messages.size shouldBe 1
                ...
                result = tryValidate { car.validate() }
                result.shouldBeSuccess()

                // just checking that everything is in order now
                result = tryValidate { car.validate(setOf(Check.DEFAULT, Check.CAR, Check.DRIVER)) }
                result.shouldBeSuccess()
            }
        }
    }) {
